...
class loginClassModel extends Model
{
	...
	public function start($user, $pass, $cfrom='', $devices='')
	{
		...
		$fields = '`pass`,`id`,`name`,`user`,`mobile`,`face`,`deptname`,`deptallname`,`ranking`,`apptx`,companyid';
		$posts  = $user;
		...
		if($msg=='' && !$us){
			...
			$exar=$this->db->getone('[Q]company', $ar , $fie);
			
			$exptime=strtotime($exar['exptime']);	
			
			 $timnow=time();			
			// echo($timnow."\t".$exptime); exit;
			 //print_r($exptime);exit();
			
			 
			
			if(!empty($exptime) && $exptime!=null)					
			{
			  	if($timnow>$exptime)				
				{
					//$this->db->update('[Q]admin',"`status`=`0`", $uid);					
					$msg="账户已过期 请联系客服: 400-845-8455";						
					return $msg;							
