...
class crmClassModel extends Model
{
	...
	{
// 		$where 	= '`uid`='.$uid.' or ( `id`='.$id.')'; // exit($where);  and `isover`=0 or
// 		$rows 	= m('custract')->getrows($where, 'id,custid,custname,money,num');
// 		return $rows;

		$rs	 = m('admin')->getone($uid,'deptid');
		$deptid = $rs['deptid'];
        $where 	= '`uid`='.$uid.' or `managerid` ='.$uid.' or `uid` IN ( SELECT `id` FROM `[Q]admin` WHERE instr( `deptpath`, \'['.$deptid.']\' )> 0 AND `deptid` != '.$deptid.' ) or `managerid` IN ( SELECT `id` FROM `[Q]admin` WHERE instr( `deptpath`, \'['.$deptid.']\' )> 0 AND `deptid` != '.$deptid.' )';
		$rows 	= m('custract')->getrows($where, 'id,custid,custname,money,num');
		return $rows;
	}
	...
	public function ractmoney($htid)
	{
		...
		$money 	= $this->db->getmou('[Q]custfina','sum(money)','htid='.$htid.' and `ispay`=1');  //收款单里已支付的
		
		$moneyy	= $this->getmoneys($htid); //已创建收付款单金额  包括 未必 已支付
		...
		if($ispay != $oispay || $symon!= $moneys || $isover != $ors['isover']){ //这里是干毛用的？？？？ what the fuck?!??
	...
	*/
	...
	// by andy.ma
	public function getfapiao($fpname="",$id=0)
	{
		...
		//$str=" uid='".$_SESSION['xinhu_adminid']."' ";
        $str=" comid='".$_SESSION['xinhu_companyid']."' ";
		//exit(  $_SESSION['xinhu_adminid'] );
		
		$rows 	= $this->getrows($str,'id as value,fpname as name,id','`name`');
	...
	// 读取账户列表  by andy.ma2019/10/11	
	public function getmyacct($acc_name="",$id=0)
	{
		...
		$whe=" and comid='".$_SESSION['xinhu_companyid']."'"; //exit($whe);
		
		$rows 	= $this->getrows("((`acc_name` like '%$acc_name%') or (`id`=$id) )  $whe ",'id as value,acc_name as name,id','`name`');
		//print_r($rows);exit('wtf');	
	...
	// 读取账户列表  by andy.ma2019/10/11	
	public function getmyacct2($acc_name="",$id=0)
	{
		...
		$rows 	= $this->getrows("((`acc_name` like '%$acc_name%') or (`id`=$id) )  $whe ",'id as value,acc_num as name,id','`name`');
		//print_r($rows);exit('wtf');	
	...
	// 获取费用类型  by andy.ma2019/10/11	
	public function getfeetype($feename="",$id=0)
	{
		...
		$this->settable('feetype');
		
			$whe=" and comid='".$_SESSION['xinhu_companyid']."'"; 
		
		$rows 	= $this->getrows("((`feename` like '%$feename%') or (`id`=$id) ) $whe ",'id as value,feename as name,id','`name`');	 		
	...
	//获取银行账号  by andy.ma
	public function getBank($acc_name="",$id=0)
	{
		if(isempt($id))$id = 0; echo $acc_name;
		
		$this->settable('account'); //exit("wtffff");
		
		$rows 	= $this->getrows("  acc_name like '%$acc_name%'   ",'id as value,acc_name as name,id','`id`');	
