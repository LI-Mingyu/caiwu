...
class flow_userClassModel extends flowModel
{
	...
	*/
	protected function flowbillwhere($uid, $lx)
	{
		...
		return array(
			'fields'=> '`name`,`id`,`id` as uid,`face`,`sort`,`deptallname`,deptpath,companyid,`ranking`,`tel`,`mobile`,`email`,`user`,num,workdate,sex,deptname,deptnames,superman,status,type,online,lastonline,isvcard,source',
			'order' => 'sort',
			'where' => $where
		);
	}
	...
	//	$dbs = m('company');
	//	$cmps = $dbs->getone($rs['companyid']);
		
		//$rs['companyid']=$cmps['name'];
		
		 if(isset($rs['companyid']))			
			$rs['companyid'] = m('company')->getmou('name',"`id`='".$rs['companyid']."'");  // && $lx==1 
		
		
		if($rs['source']==0) // by andy.ma
		 {	 $rs['source']='<font color=green>后端添加</font>'; 
		}
		else
		{
			 $rs['source']='<font color=red>前端注册</font>';
		}
	...
	public function flowdaorubefore($rows)
	{
		...
		$dbs	= m('dept');
		$py 	= c('pingyin');
		$dname	= $dbs->getmou('name', 1);if(isempt($dname))$dname = '中投开发团队';
