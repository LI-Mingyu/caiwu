...
as tiao";  
		...
		//对 已收 id 列进行替换  by andy.ma 2019/12/26
		 if($rs['type']==0)
		{
			$yssq="select IFNULL(sum(money),0) as yishou from xinhu_custfina where paytype=0 and type=0 and ispay=1  and htid='".$rs['id']."'";
			...
			$sqtiao="select ( select IFNULL(sum(money),0) as tiao from xinhu_custfinaa where  paytype=0 and type=0 and htid='".$rs['id']." ' ) - ( select  IFNULL(sum(money),0) as tiao from xinhu_custfinaa where  paytype=0 and type=1 and htid='".$rs['id']."') as tiao  "; 
