function initbody(){
	...
	// window.setInterval(_detectAccountExpiration, 1000 * 60 * 60);
  	// setTimeout(_detectAccountExpiration(),1000*15);
  	console.log(exptime);
}

function _detectAccountExpiration(){
//     console.log(exptime);
// 	console.log((Date.parse(new Date(exptime))-Date.parse(new Date()))/1000/60/60/24);
//   	let diffDay = parseInt((Date.parse(new Date(exptime))-Date.parse(new Date()))/(1000*60*60*24));
// 	console.log(diffDay);
  
// 	console.log('_detectAccountExpiration');
// 	if (document.getElementById("accountExpir") || diffDay>7) {
// 		console.log('ignore _detectAccountExpiration');
// 	} else {
// 		js.tanbody('accountExpir','服务到期提醒', 300,200,{
// 			html:'<div style="height:160px;padding:10px" align="center"><div style="margin: 50px auto;"><span id="accountExpir">您的服务即将到期, 剩余使用时间为: '+diffDay+'天。 为了不影响工作, 请及时续费!</span></div></div>'
// 		});
// 	}
}
