...
class mode_userClassAction extends inputAction{
	...
	protected function savebefore($table, $cans, $id, $addbo){
	    ...
	    $sql="select companyid from xinhu_admin where user='".$_SESSION['xinhu_adminuser']."'";
		
		$db=$GLOBALS['db'];
		
		$arr_u=$db->getAll($sql);
		
		//print_r($arr_u);exit("wtf");
		
	      //print_r($rows);exit('wtf2');
		...
		$rows['companyid']=$arr_u[0]['companyid']; // by andy.ma
