...
*	客户.合同管理
*/
class mode_pay_custractClassAction extends inputAction{
	...
	public function selectiao()
	{
		...
		$sqt=" select 
( c.money+( (select sum(money)  from xinhu_custfinaa where htid='".$rs['id']."' and type=1) - (select sum(money)  from xinhu_custfinaa where htid='".$rs['id']."' and type=1) ) )

- (select IFNULL(sum(money),0)  from xinhu_custfina  where  type=1 and ispay=1 and htid=".$rs['id'].")
	...
	protected function storeafter($table,$rows)
	{
		...
		*/
		...
		if($rows){
			...
			//调账
			$sqt="select  ( select   IFNULL(sum(money),0) from xinhu_custfinaa  where paytype=0 and type=1 and htid in ($ids) ) - ( select  IFNULL(sum(money),0) from xinhu_custfinaa  where  paytype=0 and type=1   and htid in ($ids) ) as dzjer ";   //exit($sqt);
			...
			//已收
			$sqys="select IFNULL(sum(money),0) as yishou from xinhu_custfina  where  type=1 and ispay=1  and htid in ($ids) ";
