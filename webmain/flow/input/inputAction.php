...
class inputAction extends Action 
{
	...
	*/
	public function saveAjax()
	{
		...
		$table			= $this->moders['table'];  //exit($table);
				   ...
				   for($i=0;$i<$cont;$i++)
					{
						...
						'jindu'  => $_POST['jindu_'.$i],

						'yinsday'  => $_POST['yinsday_'.$i],

						'ysjiner'  => $_POST['ysjiner_'.$i],

						'shsjiner'  => $_POST['shsjiner_'.$i],

						'kaipiao_day'  => $_POST['kaipiao_day_'.$i],	

						'daozday'  => $_POST['daozday_'.$i],

						'userid' =>$this->post('uid', $this->adminid),
		...
		if($table!="customer"){ // by andy.ma 2010/1/8
		//isonly唯一值的判断
		foreach($fieldsarr as $k=>$rs){
			$fiesd  = $rs['fields'];   
...
<table style="border-spacing: 0;border-collapse: collapse;"><tr bgcolor="#f1f1f1">'.$str1.'</tr>'.$str2.'';
		...
		$str.= '</table>';   file_put_contents("daochu.txt",$str);
