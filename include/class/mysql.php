...
abstract class mysql{
	...
	public function query($sql, $ebo=true)
	{
		...
		//file_put_contents("wtfsql.txt",$sql."\r\n",FILE_APPEND);
	...
	*/
	...
	{
		/*
		if(strpos($where,"`companyid`=1")>=0)
		{
			  
			$where=str_replace("`companyid`=1","1=1",$where); // by andy.ma 如果是id=1 管理员 
			
		}*/
		///file_put_contents("wtf3.txt",$where."\r\n",FILE_APPEND);	
	...
	public function getall($sql)
	{
		 ...
		 //print_r($_SESSION['xinhu_adminuser']);exit;  // 管理员 [xinhu_adminuser] => admin
		
		if(isset($_SESSION['xinhu_adminuser']) && $_SESSION['xinhu_adminuser']=='admin')
		{
			$sql=str_replace("`companyid`=1","1=1",$sql);	
			
		}
		
		// file_put_contents("wtf5.txt",$sql."\r\n",FILE_APPEND);	
